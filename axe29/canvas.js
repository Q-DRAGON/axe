...
class GuaCanvas extends GuaObject {
    ...
    drawScanline(v1, v2, image) {
        ...
        if (image == undefined) {
            ...
            for (let x = x1; x <= x2; x++) {
                ...
                let newvector = a.position.interpolate(b.position, factor)
                let uv = a.texture.interpolate(b.texture, factor)
                let index = parseInt(uv.x * image.width) + (parseInt(uv.y * image.height) - 1) * image.width
    ...
    project(coordVector, transformMatrix) {
        ...
        return GuaVertex.new(v, coordVector.color, coordVector.texture)
    }
    drawMesh(mesh) {
        let self = this
        // camera
